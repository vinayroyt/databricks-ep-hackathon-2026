...
test('smoke test - home page shows Care Gap Atlas regions', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: 'Regions' })).toBeVisible();
  await expect(page.getByRole('link', { name: 'Regions' })).toBeVisible();
  await expect(page.getByRole('link', { name: 'Planner AI' })).toBeVisible();
});
...
test('smoke test - planner page loads', async ({ page }) => {
  ...
  await expect(page.getByPlaceholder(/ask about care gaps/i)).toBeVisible();
});
